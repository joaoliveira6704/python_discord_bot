...
async def on_message(message):
  ...
  if message.content.startswith(prefix):
    ...
    elif message.content.lower() == prefix + "image":
      channel_id = 1314607830547955804  
      channel = client.get_channel(channel_id)
      miguel = await client.fetch_user(459481879955898380)
      if message.channel.id != 1314607830547955804:
        await message.channel.send(f"I cannot send these pictures in this channel because {miguel.mention} doesn't let me, try in {channel.mention}!")
        return
      elif message.author.id != 459481879955898380:
        await message.channel.send(f"Only {miguel.mention} can ask me for that!")
        return
      while True:
        pictures = list_files(".\\images\\")
        await message.channel.send(file=discord.File(".\\images\\"+random.choice(pictures)))
